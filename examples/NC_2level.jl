...
add_constraint!(constraints, ConstrainDiscontinuousTraversals(graph)) # ensures pieces of districts in a county are contiguous; currently required
add_constraint!(constraints, MaxCoarseNodeSplits(num_dists+1)) # fixes the maximum number of split counties
